...
Regression Trees are an alternative way to obtain estimations of $y$ from $x$. Instead of modelling how $y$ changes with every unit change in $x$ with a function $y=f(x)$, regression trees instead use stratification:

1.  Tree-based methods will divide the independent variable $x$ into a number of regions (usually 2).
...
3.  Predict $y$, based on the mean of the region of $x$ which the observation falls into.
...
-   This is very similar to OLS, which finds coefficient values that minimises the sum of squared errors.

Once we have split $x$, find the average $y$ in each region, and that is our prediction for the points in the region.
...
7.  For each final subregion, find the average $y$ value, and that is our prediction for all points in that region.
...
-   Notice how after the top division, we go to divide the subregions. We always have one divison per "level".

-   Notice how the variable **taxpercent** appears again on the right hand side. Trees test optimal $x$ and $s$ with replacement.
...
-   In a set of $n$ observations $Z_1, Z_2, ... , Z_n$, each with variance $\sigma^2$, the variance of the mean $\bar{Z}$ of the observations is $\sigma^2/n$

-   What this means, essentially, is that averaging a set of observations will reduce the variance.

However, there is one issue: we only have one observation (our training data). How do average a set of observations if we only have one observation?
...
-   Essentially, we sample observations from our current data set, and once we draw one observation out, we put it back in before drawing the next one.

-   If we take a few Bootstrap Samples, we will have a few similar, but slightly different data sets. We are essentially replicating the process of obtaining new data sets, without actually gathering more data.
...
-   We then train a tree on each different sample, creating a prediction function $f_b(x)$, where $b$ represents the $b$th sample.

-   Then, we average all the predictions to obtain our prediction function: $f_{bag}(x)=\frac{1}{B} \sum_{b=1}^{B} f_b(x)$
...
Random Forest address this issue by not only Bootstrap Sampling observations like Bagging does, but also Bootstrap Sampling predictor $x$ variables.
