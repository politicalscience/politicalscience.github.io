...
2 assumptions are needed for identification:
...
Using these two assumptions, we can first identify the CATE, which will allow us to identify the ATE and ATT. Let us first start with the definition of CATE (@def-cate):
...
And the second step above is because $\pt|D_i = 1$ and $\pc|D_i = 0$ are observable outcomes (@def-observedoutcomes). Thus, with independence, we can identify the CATE with just observed outcomes. With the CATE identified, we can now identify both the ATE and ATT.

::: {.panel-tabset .nav-pills}
## ATE Identification

Let us consider the definition of the ATE, which is $\E(\pt - \pc)$. From the definition of a continuous random variable's expectation given in @def-exp, we can rewrite the ATE as
...
Let us consider the definition of the ATT, which is $\E(\pc = \pt | D_i = 1)$. From the definition of a continuous random variable's expectation given in @def-exp, we can rewrite the ATT as

$$
\tau_{ATT} = \int \tau_{CATE}(x) \P(x|D_t = 1) dy
$$

Which is a weighted average. We established in @eq-soocate that we can identify the CATE. Thus, we can plug @eq-soocate into the ATE to get our identified ATT:
...
The idea of the non-compliance designs is to use our encouragement/treatment assignment $Z$ as an [instrument](ols.qmd#instrumental-variables-estimator) for $D$ - actually taking the treatment. Our causal estimands are:
...
::: {#def-noncomplianceiv style="border-style: solid; border-radius: 8px; border-width: 0.5px; padding-top: 5px;   padding-right: 5px; padding-bottom: 5px; padding-left: 5px;"}
## Non-Compliance IV

There are 4 assumptions to identify the causal estimands:
...
Thus, we have identified the LATE under the assumptions from @def-noncomplianceiv. We will generally estimate the LATE with the 2-stage-least-squares (2SLS) estimator.
