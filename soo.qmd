...
subtitle: "Chapter 5, Quantitative Methods"
sidebar: side
---
...
2.  The nodes within set $X$ [block all back-door paths](causal.qmd#blocked-paths) from $D \rightarrow Y$.
...
With our assumptions above, we can identify the ATE. We start with the [conditional average treatment effect](causal.qmd#conditional-average-treatment-effect-cate), conditional on some value of confounders $X_i = x$. Note the properties shown in @eq-independence .
...
OLS is a natural approach for controlling for confounders $X$, since $\hat\beta_{OLS}$ estimates [partial out the effects of covariates](clm.qmd##regression-anatomy-and-controlling). OLS is a good estimator of $\tau_{ATE}$ under 2 conditions:
...
Thus, we see $\tau_i = \beta_1$. We also know that conditional ignorability implies [zero-conditional mean](#0). Thus OLS is an unbiased and asymptotically consistent estimator of both $\beta_1$ and the ATE.

You should be cautious using OLS when assumption 2, **linearity**, is violated. We know the OLS is the [best linear predictor of the conditional expectation function](clm.qmd) in terms of mean squared error. Thus, $\beta_1$ will provide the best linear approximation to the population regression function.
...
What if there are **heterogenous** treatment effects (where $\tau_i$ is different between units)? Standard OLS in this case is [**no longer** an unbiased estimator]{.underline} of the ATE. Recall the discrete identification of the ATE (in equation @eq-discreteate ) is a weighted average of CATEs:
...
1.  Choose one specific value for all covaraites $X$. Find the ATE within this specific value of $X$.
...
We have a [missing data problem](causal.qmd#observed-outcomes-and-missing-data) in causal inference: we do not know all the potential outcomes. Matching "estimates" missing potential outcomes of a unit.
...
![](images/clipboard-2668193698.png){fig-align="center" width="60%"}
...
This curse of dimensionality creates a [bias](stats.qmd#finite-sample-properties-of-estimators) problem - since we get non-exact matches. The more dimensions you add, the worse it becomes.
...
However, we do not actually observe $\pi (X_i)$. We estimate $\pi (X_i)$ with a [logistic regression](glm.qmd#logsitic-regression-model) (or other binary response model), with outcome variable $D_i$, and explanatory variables $X_i$. This will get us a fitted probability $\P(D_i = 1) = \hat\pi(X_i)$.
...
The IPW estimator is [asymptotically consistent](stats.qmd#asymptotic-properties-of-estimators), but has very poor small sample properties. They are highly sensitive to extreme values of $\hat\pi(X_i)$. This generates high variance (inefficiency), and can produce significant bias under model mispecification.
