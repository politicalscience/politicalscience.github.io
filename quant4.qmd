...
With our assumptions above, we can identify the ATE. We start with the [conditional average treatment effect](quant2.qmd#conditional-average-treatment-effect-cate), conditional on some value of confounders $X_i = x$. Note the properties shown in @eq-independence .
...
OLS is a natural approach for controlling for confounders $X$, since $\hat\beta_{OLS}$ estimates [partial out the effects of covariates](quant1.qmd##regression-anatomy-and-controlling). OLS is a good estimator of $\tau_{ATE}$ under 2 conditions:
...
You should be cautious using OLS when assumption 2, **linearity**, is violated. We know the OLS is the [best linear predictor of the conditional expectation function](quant1.qmd#conditional-expectation-function) in terms of mean squared error. Thus, $\beta_1$ will provide the best linear approximation to the population regression function.
