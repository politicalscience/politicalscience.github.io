...
& \underbrace{c(1, \theta_H) = 1}_{\theta_H \text{ effort cost}} \ < \ \underbrace{c(1, \theta_L) = 2}_{\theta_L \text{ effor cost} }
\end{split}
$$
...
$$
\underbrace{ty_1 + ty_2 + \dots + ty_n}_{\text{taxes from citizens}} = n t \bar y = g
$$
...
& \underbrace{\left( \frac{q^b}{1-q^a}\right)}_{\text{3's signal}} \underbrace{\frac{1-q^b}{q^a}\left( \frac{q^b}{1-q^a}\right)}_{\text{other's signals}} ≥ 1
\end{split}
$$
