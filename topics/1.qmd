...
Ansolabehere, Snyder, and Stewart (2001) surveyed some candidates from 1996. The area between the two lines is average policy of each party. Clearly, the average policies of both parties are not converging - there is some distance between them.
...
Let us say our median voter is still at the point $q^m = 0$. All voters receive a common valence shock $\delta$. This $\delta$ tells us something about how the voter $i$ feels about party $A$. This $\delta$ is uniformly distributed as following:
...
Thus, we know $p_A \in [q^A, q^m]$. And since this game is symmetrical, we know $p_B \in [q^m, q^B]$. What does this tell us? We know $q^A$ will be negative, and $q^B$ will be positive. Thus, we know $|p_A - q^A| = p_A - q^A$, and $|p_B - q^A| = p_B - q^A$. Thus, we can rewrite our utility function:
