...
In the previous chapters, we have covered estimators that primarily deal with linear models. However, not every model is linear.

In this chapter, we cover the Maximum Likelihood Estimator (MLE), the most commonly used estimator in statistics, especially commonly used for non-linear models. We discuss the intuition and mathematics behind the MLE, and how statistical inference is conducted with the MLE.

<br />

## Likelihood Function

So far, the estimators we have covered can only work with the linear model in the form $\b y = \b{X\beta} + \b\eps$. However, a lot of [data generating processes](inference.qmd#data-generating-process) do not take this form.

The maximum likelihood estimator allows us to estimate parameters $\theta$ that are not in the linear model form - as long as we know the form/probability density function of the data generating process. MLE also does not have to meet spherical errors, and only needs to meet the assumptions of [i.i.d.](ols.qmd#assumption-2-independent-and-identically-distributed-i.i.d.) and [exogeneity](ols.qmd#assumption-4-strict-exogeneity) (proper model specification).
...
## OLS as a Maximum Likelihood

Previously, we discussed the [ordinary least squares estimator](ols.qmd) used on the [classical linear model](ols.qmd#the-classical-linear-model). We can actually show that the OLS estimator is a maximum likelihood estimator when we assume a linear data generating process
...
Where $\b x_t$ is a vector $\b x_t = (1, X_{t1}, \dots, X_{tp}$ and $\b\beta$ is a vector $\b\beta = (\beta_0, \dots, \beta_p)$. Using the probability density function of a normal distribution (@def-normalpdf) , we can express the PDF of $Y_i$ as:

$$
f_{Y_t}(y_t) = \frac{1}{\sqrt{2\pi\sigma^2}}e^{-\frac{1}{2\sigma^2}(y_t - \b x_t^\top \b\beta)^2}
$$

Now, that means our log-likelihood $\ell$ (@def-loglike) for our sample $(y_1, \dots, y_n)$ we have is:
...
If we look at the blue, we can see this is the sum of squared residuals (@def-ssr). Thus,
...
For testing multiple coefficients at once, like the f-test for OLS (@def-ftest), we can use the Likelihood Ratio test. Like the f-test, the likelihood ratio test compares two models:
