...
subtitle: "Multivariate Measurement"
format:
    html:
        page-layout: article
        grid:
...
This chapter introduces principle components analysis, a very frequently used dimensional-reduction approach that takes many variables, and condenses them down into a few smaller variables that still retain much of the variance of the original ones.
...
1.  Each principle component is a linear combination (weighted sums) of the original variables: $y_j = a_{1j}x_1 + a_{2j}x_2 + \dots + a_{pj}x_p$.
2.  The principle components together have the same total variance as the original variables: $\sum Var(y_i) = \sum Var(x_i)$. Thus, the principle components carry the same information/variation as the original variables, just with a different distribution.
...
-   With $a_{ij}$ being the weights of the linear combinations.

We can rewrite each principle component $y_j$ in terms of linear algebra:

$$
y_j = \mathbf a_j^T\mathbf x = \underbrace{\begin{pmatrix}
...
$$
\mathbf y = \mathbf A^T \mathbf x, \quad \mathbf A = \begin{pmatrix} \mathbf a_1 & \mathbf a_2 & \dots & \mathbf a_p \end{pmatrix}
$$

Each component is a weighted sum of the original variables, with weights $a_{ij}$. These weights have a few properties:

1.  $\sum_i a_{ij}^2 = 1$ for each $j = 1, 2, \dots, p$. Or in other words, the weights for each principle component $y_j$ should sum to 1.
...
The variance of $y_j$, is equivalent to the $j$th eigenvalue:
