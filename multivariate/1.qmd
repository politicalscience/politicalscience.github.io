...
subtitle: "Methods for Measurement"
format:
    html:
        page-layout: article
        grid:
...
The sample correlations $Corr(x_i, x_k)$ can be summarised in a covariance matrix. We notate the covariance matrix as $\boldsymbol\Sigma$. We can also use a correlation matrix (more on that later).
...
We can rewrite each principle component in terms of linear algebra:
...
And all principle components can be expressed as:
