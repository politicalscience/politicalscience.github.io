...
We know that $x_i$ is related to $\xi$ by $x_i = \tau_i + \lambda_i \xi + \delta_i$, a linear regression. Thus, $E(x_i|\xi) = \tau_i + \lambda_i \xi$. Since $x_i$ is also normally distributed, we can say $\xi \sim \mathcal N(\tau_i + \lambda_i \xi, \theta_{ii})$.
