...
Without randomisation, we need some other way to account for pre-treatment covariates that may be confounding and causing selection bias.
...
For selection on observables, the idea is to control for these pre-treatment covariates. The assumptions needed for identification of causal effects are:

1.  **Conditional Ignorability** (also known as exogeneity or independence): Among units with identical pre-treatment covariate values $X_i$, treatment $D_i$ is as-if randomly assigned. Or in other words, potential outcomes are independent from treatment within each specific covariate value $X_i = x$.
...
1.  No node within set $S$ is a descendant of $X$ (no element within $S$ results from $D$):
2.  The nodes within set $S$ [block all back-door paths](https://politicalscience.github.io/causal/2.html#blocked-paths) from $X \rightarrow Y$.
...
1.  $\{Z, M\}$: M blocks the lower path $D \rightarrow M \rightarrow Y$, and $Z$ blocks the upper path $D \rightarrow Z \rightarrow M \rightarrow Q \rightarrow Y$.
2.  $\{M, Q\}$:M blocks the lower path $D \rightarrow M \rightarrow Y$, and $Q$ blocks the upper path $D \rightarrow Z \rightarrow M \rightarrow Q \rightarrow Y$.
3.  $\{Z, Q, M\}$: M blocks the lower path $D \rightarrow M \rightarrow Y$, and $Z, Q$ blocks the upper path $D \rightarrow Z \rightarrow M \rightarrow Q \rightarrow Y$.
...
-   Where $M$ is the number of strata (levels/categories of $X$), where $n_j$ is the number of units in a level/category $j$ of $X$, and where $\bar Y_{dj}$ is the mean outcome for units with $D_i = d$ in level/category $j$ of $X$.
...
We have a [missing data problem](https://politicalscience.github.io/causal/2.html#observed-outcomes-and-missing-data) in causal inference: we do not know all the potential outcomes. Matching "determines" missing potential outcomes of a unit, by using the closest unit or nearest neighbour to the unit in question.

For each observation in the treated group, matching finds an observation in the untreated group that have the most similar values of a set of pre-treatment covariates $X$. Thus, we have pairs of observations. We can estimate the ATT as the average difference in observed outcomes between the pairs:

$$
\hat\tau_{ATT} = \frac{1}{n_1} \sum\limits_{i:D_i = 1}(Y_i - \widetilde{Y_i})
$$

Where $n_1$ is the number of units in the treatment group, $Y_i$ is the unit's observed $Y$ in the treatment group, and $\tilde Y_i$ is unit $i$'s closest neighbour in the untreated group.
...
Consider that we have a more than one $X$ pre-treatment covariate. This implies that $X$ values are in a multidimensional space $\mathbb R^k$, where $k$ is the number of covariates.

The most commonly used Distance Metric is Mahalanobis Distance - which measures the distance in $X$ between units $i$ and $j$:
...
The figure on the left illustrates this. $d$ represents the dimensions. We can see as the dimensions increase, the fraction of volume increases significantly slower relative to distance.
...
Propensity Score matching is an alternative way to match over many dimensions.

The propensity score is an unobserved property, defined as the probability of a unit $i$ of receiving treatment:

$$
\pi(X_i) \equiv Pr(D_i = 1|X_i)
$$

When supposing the conditional ignorability and common support assumptions, the propensity score $\pi(X_i)$ has the balancing property: $D_i \perp X_i \ | \ \pi(X_i)$. This implies that conditional ignorability holds, just on the propensity scores alone:

$$
(Y_{1i}, Y_{0i}) \perp D_i \ | \ \pi(X_i)
$$

Thus, instead of conditioning on $X_i$ as we did in selection on observables, we can instead condition on $\pi (X_i)$, and still identify the causal estimands.

However, we do not actually observe $\pi (X_i)$. Generally, we estimate $\pi (X_i)$ with a logistic (or other binary response variable), with outcome variable $D_i$, and explanatory variables $X_i$. This will get us a predicted proability $Pr(D_i = 1) = \hat\pi(X_i)$.

Then, once we have the propensity score estimates $\hat\pi(X_i)$, we can do nearest neighbour matching with the propensity scores. This will allow us to identify the $\tau_{ATT}$.
...
An alternative use of the propensity score is weighting. Under both conditional ignorability and common support assumptions, we can identify the ATE and ATT as:
...
Above in propensity scores, we showed that the conditional ignorability assumption is true with propensity scores:

$$
(Y_{1i}, Y_{0i}) \perp D_i \ | \ \pi(X_i)
$$

Using this, let us reason about the CATE:

$$
\begin{split}
\tau_{CATE}(\pi(X_i)) & = E(Y_{1i} - Y_{0i} \ | \pi(X_i)) \\
& = E(Y_{1i}| \pi(X_i)) - E(Y_{0i}|\pi(X_i)) \\
& = E(Y_{1i}| D_i = 1, \pi(X_i)) - E(Y_{0i}|D_i = 0, \pi(X_i)) \\
& = E(Y_i|D_i = 1, \pi(X_i)) - E(Y_i |D_i = 0, \pi(X_i))
\end{split}
$$

Now, let us reason about the ATE, and plug in the CATE when applicable:
:::

The **inverse probability weighting (IPW) estimator** would be:
...
### OLS under Constant Treatment Effect

When we think about controlling for covariates, we often think about regression. OLS is a natural approach, since $\hat\beta_{OLS}$ estimates [partial out the effects of covariates](https://politicalscience.github.io/causal/1.html#regression-anatomy-theorem).

$\hat\beta_{OLS}$ a good estimator of $\tau_{ATE}$ in selection of observables under 2 conditions:
...
We also know that under selection on observables, conditional ignorability implies zero-conditional mean. Thus $\beta_1$ is unbiased and asymptotically consistent.
...
### OLS under Heterogenous Treatment Effects

Remember that the OLS is only an unbiased estimator of the ATE when there are constant treatment effects for all units, as mentioned above. What if there are heterogenous treatment effects (where $\tau_i$ is different between units)?

Recall the subclassification estimator of the ATE:

$$
\hat\tau_{ATE} = \sum\limits_{x \in \mathcal X} ( E(Y_i|D_i = 1, X_i = x) - E(Y_i|D_i = 0, X_i = x)) \underbrace{Pr(X_i = x)}_{\text{weight}} \\
$$

OLS, when there are non constant treatment effects, can be rewritten as:

$$
\hat\beta_{OLS} = \sum\limits_{x \in \mathcal X} ( E(Y_i|D_i = 1, X_i = x) - E(Y_i|D_i = 0, X_i = x)) \underbrace{ \frac{Var(D_i|X_i = X)Pr(X_i = x)}{\sum Var(D_i | X_i = x')Pr(X_i = x')} }_{\text{weight}} \\
$$

Notice how the weights are different. The weights in the OLS are the conditional variances of $D_i$.

This means the OLS actually provides an unbiased estimator of the **conditional variance weighted average treatment effect**. This is not the same as the ATE or the ATT. Thus, we should be careful when using this estimator with heterogenous treatment effects.

::: {.callout-note collapse="true" appearance="simple"}
## Conditional Variance Weighted Average Treatment Effect
...
The reason for this is because the OLS is predicting missing potential outcomes for the treated units.
...
The Fully-Interacted Estimator, a newly developed large-sample regression estimator is as follows:

$$
Y_i = \hat\alpha + D_i \widehat{\tau}_{int} + (\mathbf X_i - \mathbf {\bar X}) \hat\beta +D_i (\mathbf X_i - \mathbf{\bar X}) \hat\gamma
$$

Where $X_i$ are covariate values sufficient to satisfy conditional independence, and $\bar X$ contains the sample means of all $X_i$ covariates.
...
### Choosing an Estimator and Modern Advances

There are advantages and disadvantages to all the estimators discussed.

Regression is simple. However, regression also relies on many strong assumptions of linearity and uniform treatment effects. The fully-Interacted estimator can help with this.

Regression is also prone to extrapolation beyond common support - i.e. it can "estimate" potential outcomes for units that are not observed. This can lead to bias.

The advantage of matching is that it does not have model assumptions of linearity. It is also quite intuitive of an idea. However, the inability to find exact matches can cause bias, especially for the more covariates we match on.

Modern approaches sometimes combine regression with matching or weighting:

1.  **Bias-corrected matching** estimates bias ineherent to mathching estimators via regression, then subtracts it from the matching estimate to correct for it.
2.  **Doubly-robust estimation** uses a weighted average of regression and IPW estimators, which will be asymptotically consistent as long as the regression model is correctly specified.
3.  **Matching as pre-processing** uses matching to make treatment and control groups similar, then runs regression models to estimate causal effects.
