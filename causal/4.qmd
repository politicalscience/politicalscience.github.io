...
This implies that for any given value of all confounders $X_i = x$, we know:
...
When we think about controlling for covariates $X$, we often think about regression. OLS is a natural approach, since $\hat\beta_{OLS}$ estimates [partial out the effects of covariates](https://politicalscience.github.io/causal/1.html#regression-anatomy-theorem).
...
We also know that under selection on observables, conditional ignorability implies [zero-conditional mean](https://politicalscience.github.io/causal/1.html#conditions-for-unbiasedness). Thus $\beta_1$ is an unbiased and asymptotically consistent estimator of the ATE.
...
What if there are **heterogenous** treatment effects (where $\tau_i$ is different between units)? OLS in this case is no longer an unbiased estimator of the ATE.

Recall the subclassification estimator of the ATE:

$$
\hat\tau = \sum\limits_{x \in \mathcal X} ( E(Y_i|D_i = 1, X_i = x) - E(Y_i|D_i = 0, X_i = x)) \underbrace{Pr(X_i = x)}_{\text{weight}} \\
$$

OLS, when there are non constant treatment effects, can be rewritten as:

$$
\hat\beta = \sum\limits_{x \in \mathcal X} ( E(Y_i|D_i = 1, X_i = x) - E(Y_i|D_i = 0, X_i = x)) \underbrace{ \frac{Var(D_i|X_i = X)Pr(X_i = x)}{\sum Var(D_i | X_i = x')Pr(X_i = x')} }_{\text{weight}} \\
$$
...
We must be careful of this property when using the OLS estimator for heterogenous treatment effects.
...
::: {.callout-note collapse="true" appearance="simple"}
## Solutions to the OLS Bias under Heterogeneity

There are a few solutions to this issue of OLS bias under heterogeneity:
...
Finally, there is the fully interacted estimator, which is explained below.
:::
...
The Fully-Interacted Estimator, a newly developed large-sample regression estimator, solves the heterogeneity bias in the OLS estimator. The fully-interacted estimator takes the form:
