...
Let us return to our [naive estimator](https://politicalscience.github.io/causal/1.html#naive-estimator-and-selection-bias), and our problem of selection bias. Using the above property in @eq-independence, we can simplify:

$$
\begin{align}
\hat\tau_{naive} & = \underbrace{E(Y_{1i}|D_i = 1)- E(Y_{0i}|D_i = 1)}_{\tau_{ATT}} + \underbrace{E(Y_{0i}|D_i = 1) - E(Y_{0i} | D_i = 0)}_{\text{Selection Bias}} \\
...
Thus, under randomisation, selection bias is equal to 0, and thus our comparison of observed outcomes is now an unbiased estimator of $\tau_{ATT}$. Now look at the formula for the [ATT](https://politicalscience.github.io/causal/1.html#average-treatment-effect-on-the-treated-att) estimand. We can simplify as follows using @eq-independence:
...
This is an unbiased estimator because selection bias is eliminated with randomisation. This is also an asymptotically consistent estimator due to the [law of large numbers](https://politicalscience.github.io/causal/0.html#asymptotically-consistent-estimators).
...
We do not need to include covariates. This is because randomisation allows us to meet the [asymptotic consistency](https://politicalscience.github.io/causal/0.html#conditions-for-asymptotic-consistency) condition of both randomisation and exogeneity.
...
We can also use [Nonparametric Bootstrap](https://politicalscience.github.io/causal/0.html#nonparametric-bootstrap) to create our sampling distribution for statistical inference, instead of relying on asymptotic normality of the standard t-test.
