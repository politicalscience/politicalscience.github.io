...
If set $S$ of nodes blocks all paths from $D$ to $Y$, then set $S$ of nodes d-separates $D$ and $Y$.

![](images/clipboard-39157862.png){fig-align="center" width="40%"}

We can see here that $D$ and $Y$ are d-separated by:

1.  $\{Z, M\}$: The path would be $D \rightarrow Z \rightarrow M \rightarrow Q \rightarrow Y$. $Z$ is an arrow emitting node, so it meets condition 1 of a blocked path. $M$ is a collision node
2.  $\{Q, M\}$:
3.  $\{Z, Q, M\}$:
