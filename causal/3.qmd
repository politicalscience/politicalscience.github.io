...
\begin{split}
\tau_{ATE}  = & E(Y_{1i} - Y_{0i}) \\
 = & \underbrace{E(Y_{1i} - Y_{0i} | D_i = 1) Pr(D_i = 1) - E(Y_{1i} - Y_{0i}|D_i = 0)Pr(D_i = 0)}_{\because \text{ weighted average}} \\
= & [ \overbrace{E(Y_i |D_i = 1)}^{\text{observed}} - \overbrace{E(Y_{0i}|D_i = 1)}^{\text{unobservered}}] Pr(D_i = 1) \\
& + [ \underbrace{E(Y_{1i}|D_i = 0)}_{\text{unobserved}} - \underbrace{E(Y_i|D_i = 0)}_{\text{observed}}]Pr(D_i = 0)
\end{split}
$$

Some of the quantities are observed, and some of the quantities are unobserved. Previously, we made assumptions (conditional ignorability, common support) to fill the unobserved quantities. But, we can make actually any assumption as possible.
...
$$
\underline \tau = [ \underbrace{E(Y_i |D_i = 1)}_{\text{observed}}  - \color{red}{Y_H} \color{black}] Pr(D_i = 1)
+ [ \color{red}{Y_L} \color{black} - \underbrace{E(Y_i|D_i = 0)}_{\text{observed}}]Pr(D_i = 0)
$$
...
$$
\overline \tau = [ \underbrace{E(Y_i |D_i = 1)}_{\text{observed}}  - \color{red}{Y_L} \color{black}] Pr(D_i = 1)
+ [ \color{red}{Y_H} \color{black} - \underbrace{E(Y_i|D_i = 0)}_{\text{observed}}]Pr(D_i = 0)
$$

We know that the true $\tau_{ATE} \in [\underline \tau, \overline \tau]$
...
$$
\begin{split}
\overline \tau = & [ \underbrace{E(Y_i |D_i = 1)}_{\text{observed}}  - \color{red}{E(Y_{0i}|D_i = 0)} \color{black}] Pr(D_i = 1) \\
& + [ \color{red}{E(Y_{1i}|D_i =1)} \color{black} - \underbrace{E(Y_i|D_i = 0)}_{\text{observed}}]Pr(D_i = 0) \\
