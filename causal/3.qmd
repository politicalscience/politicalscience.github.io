...
& = \underbrace{E(Y_{1i} )}_{\text{independence}} - \underbrace{E(Y_{0i})}_{\text{independence}} \\
...
We can test this assumption by finding the average $X$ values for both control and treatment groups, and see if there are any statistical significant differences in $X$ between control and treatment.
