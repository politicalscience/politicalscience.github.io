...
This chapter introduces the selection on observables framework, which allows us to identify causal effects by controlling for observable pre-treatment covariates. We discuss the main estimators, including subclassification, regression, and matching.
...
![](images/clipboard-378311705.png){fig-align="center" width="45%"}
...
With our assumptions above, we can identify the ATE. We start with the [conditional average treatment effect](https://politicalscience.github.io/causal/1.html#conditional-average-treatment-effect-cate), conditional on some value of confounders $X_i = x$. Note the properties shown in @eq-independence .
...
OLS is a natural approach for controlling for confounders $X$, since $\hat\beta_{OLS}$ estimates [partial out the effects of covariates](https://politicalscience.github.io/causal/0.html#regression-anatomy-theorem). OLS is a good estimator of $\tau_{ATE}$ under 2 conditions:
...
We also know that conditional ignorability implies [zero-conditional mean](https://politicalscience.github.io/causal/0.html#ols-as-an-unbiased-estimator). Thus $\beta_1$ is an unbiased and asymptotically consistent estimator of the ATE.
...
The stronger (bolder) our assumptions for identification, the less credible our results are. Selection on Observables involves a very strong and hard to verify assumption: [conditional ignorability](https://politicalscience.github.io/causal/3.html#identification-assumptions). Can we really be sure that we have controlled for all confounders $X_i$ needed to satisfy conditional ignorability?
