...
This background chapter introduces some key econometric theory needed for causal inference. Topics covered includes properties of estimators, the OLS estimator, and the Method of Moments estimator.
...
However, because of sampling variability (not all random samples will be identical), each sample $n$ different estimate $\hat\theta_n$.
...
& \mathbf X^T \mathbf  u = \sum\limits_{i=1}^n \mathbf x_i u_i
\end{split}
$$
:::
...
$$
\begin{align}
\boldsymbol{\hat\beta} & = \boldsymbol\beta + \left( \sum\limits_{i=1}^n \mathbf x_i \mathbf x_i^T\right)^{-1} \left( \sum\limits_{i=1}^n \mathbf x_i \mathbf u \right) && (\text{vector notation})\\
\boldsymbol{\hat\beta} & = \boldsymbol\beta + \left( \frac{1}{n}\sum\limits_{i=1}^n \mathbf x_i \mathbf x_i^T\right)^{-1} \left( \frac{1}{n}\sum\limits_{i=1}^n \mathbf x_i \mathbf u \right) && ( \ \left(\frac{1}{n} \right)^{-1} \text{and } \frac{1}{n} \text{ cancel out}) \\
\text{plim} \boldsymbol{\hat\beta} & = \boldsymbol\beta + \left( \text{plim} \frac{1}{n}\sum\limits_{i=1}^n \mathbf x_i \mathbf x_i^T\right)^{-1} \left( \text{plim} \frac{1}{n}\sum\limits_{i=1}^n \mathbf x_i u_i \right) && (\text{apply plim})
\end{align}
$$

And using the [law of large numbers](https://politicalscience.github.io/causal/0.html#asymptotically-consistent-estimators), and the zero-mean and exogeneity condition, we can simplify to:

$$
\begin{align}
\text{plim} \boldsymbol{\hat\beta} & = \boldsymbol\beta + (E(\mathbf x_i \mathbf x_i^T))^{-1}E(\mathbf x_i  u_i) && (\text{law of large numbers})\\
