...
In statistics, we will talk a lot about random variables. Random variables are outcomes that have some randomness/uncertainty. For example, if you flip a coin, you could get heads or tails. While we do not know the outcomes of these random events, we do know at what probability things will happen. You know that if you flip a coin, you have a 50% chance you get heads.
...
**Continuous random variables** can take any possible value within a range, including 1, 2, 1.33247. **Discrete random variables** can only take a certain set of values, not any possible decimal. For example, a dice roll can only take the values of 1, 2, 3,..., and not 3.23478. A discrete random variable with only 2 potential outcomes is a **binary random variable**.

<br />

### Probability Density Functions {#probability-density-functions}

We can describe these distributions mathematically. A **probability density function** is a function $p$ that takes one potential outcome as an input, and spits out the probability of that outcome. For example, the probability density function of a dice is:
...
$$
\sim \mathcal N(\mu, \sigma^2), \quad PDF: Pr(a<x<b) = \int\limits_a^b \frac{1}{\sqrt{2\pi\sigma^2}}e^{\left( -\frac{(x - \mu)^2}{2 \sigma^2}\right)}dx
$$
...
### Finite Sample Properties of Estimators {#unbiased-estimators}
...
### Asymptotic Properties of Estimators {#asymptotically-consistent-estimators}
