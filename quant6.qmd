...
OLS is a natural approach for controlling for confounders $X$, since $\hat\beta_{OLS}$ estimates [partial out the effects of covariates](quant3.qmd#regression-anatomy-theorem). OLS is a good estimator of $\tau_{ATE}$ under 2 conditions:
...
This curse of dimensionality creates a [bias](quant3.qmd#unbiased-estimators) problem - since we get non-exact matches. The more dimensions you add, the worse it becomes.
...
The IPW estimator is [asymptotically consistent](quant3.qmd#asymptotically-consistent-estimators), but has very poor small sample properties. They are highly sensitive to extreme values of $\hat\pi(X_i)$. This generates high variance (inefficiency), and can produce significant bias under model mispecification.
