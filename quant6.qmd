...
We know the OLS is the [best linear predictor of the conditional expectation function](quant2.qmd#conditional-expectation-function) in terms of mean squared error. Thus, $\beta_1$ will provide the best linear approximation to the population regression function.
...
This curse of dimensionality creates a [bias](quant1.qmd#finite-sample-properties-of-estimators) problem - since we get non-exact matches. The more dimensions you add, the worse it becomes.
...
The IPW estimator is [asymptotically consistent](quant1.qmd#asymptotic-properties-of-estimators), but has very poor small sample properties. They are highly sensitive to extreme values of $\hat\pi(X_i)$. This generates high variance (inefficiency), and can produce significant bias under model mispecification.
