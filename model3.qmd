---
title: "Spatial, Temporal, and Big Data"
subtitle: "Further Statistical Models"
sidebar: side
---

This chapter covers methods for different data types. We start with models for spatial data. Then we move to models for time series data. Then finally, we move into simple machine learning methods for big data, including random forest and naive bayes.
...
1.  **Lag y**: The outcome $y$ of a region might be affected by a neighbouring region's $y$ values. For example, one state having a high crime rate may affect another nearby state's crime rate as criminals can move across the border.
2.  **Lag X**: The outcome $y$ of a region might be affected by a neighbour's $X$ values. For example, one state's low vaccination rate could affect the prevalence of a disease in a nearby state.
3.  **Spatial Autocorrelation**: The residuals $u$ of a region might be affected by the residuals in a neigbouring region. For example, perhaps if we are studying how minimum wage affects unemployment rates, there could be some common factor with nearby states that affects unemployment rates (for example, prevalence of a certain industry) which is not included in our model.
...
There are other ways to define neighbours $w_{ij} = 1$ outside of just borders. For example, we could consider any region who's centroid is within $x$ miles of another state's centroid as a neighbour. This is up to the modelers discretion.
...
$$
\mathbf y = \underbrace{\rho\mathbf W\mathbf y}_{lag \ y} + \mathbf X\boldsymbol\beta + \underbrace{\mathbf{WX}\boldsymbol\theta}_{lag \ x} + \underbrace{\lambda \mathbf{Wu}}_{autocorr.} + \boldsymbol\epsilon
$$

-   $\rho$ (scalar) is the lag $y$ coefficient, and measures how an average of neighbouring region's $y$ values are associated with a region's own $y$ values. For every one unit increase in the average value of $y$ in neighbouring regions, there is an expected $\rho$ unit change in $y$ in the region, holding all else constant.
-   $\boldsymbol\beta$ (vector) is our normal OLS coefficients $\beta_0, \dots, \beta_k$, and would be interpreted in the same way.
-   $\boldsymbol\theta$ (vector) is the lag $X$ coefficients $\theta_0, \dots, \theta_k$, and measures how an average of neighbouring region's $X$ values are associated with a region's own $y$ values. For every one unit increase in the average $x_j$ of neighbouring regions, there is an expected $\theta_j$ unit change in $y$ in the region, holding all else constant.
-   $\lambda$ (scalar) is spatial autocorrelation coefficient, and measures how an average of neighbouring regions' error terms are associated with a region's own error term. Unobserved factors shared by neighbouring regions contribute to a $\lambda$-unit change in $y$, holding all else constant.
...
1.  Keleijian-Prucha Model: $\mathbf y = \rho \mathbf W \mathbf y + \mathbf X \boldsymbol\beta + \lambda \mathbf {Wu} + \boldsymbol\epsilon$.
2.  Spatial Durbin Model: $\mathbf y = \rho \mathbf{WY} + \mathbf X \boldsymbol\beta + \mathbf{WX}\boldsymbol\theta + \boldsymbol\epsilon$.
3.  Spatially Lagged X Model: $\mathbf y = \mathbf X \boldsymbol\beta + \mathbf {WX} \boldsymbol\theta + \boldsymbol\epsilon$.
4.  Spatial Lag Model (Spatial Autoregressive): $\mathbf y = \rho \mathbf{Wy} + \mathbf X \boldsymbol\beta + \boldsymbol\epsilon$.
5.  Spatial Error Model: $\mathbf y = \mathbf X\boldsymbol\beta + \lambda \mathbf{Wu} + \boldsymbol\epsilon$.

The models are estimated with Maximum Likelihood Estimation, and model selection can be done with a combination of theory regarding the topic, eliminating insignificant portions, and AIC and BIC metrics.
...
One important idea is a **stationary process**, where there is zero trend over time.

-   A strong stationary process essentially means every single time period $X_t$ is independent of each other - random draws from the same distirbution. This is unlikely to be true for most time-series data.
-   A weak stationary process is when the expected value of our time series is constant over different groups of time periods: $E(X_t) = \mu$.

Most time-series data will not be either strong or weak stationary. However, we can transform most time-series data into weak stationary processes through **differencing**. We refer to the new differenced time series as $X'_t$:
...
Any further order of differencing $d$ is increasingly uncommon. The point of differencing is that we keep increasing $d$ until we have a weak stationary process.

<br />

### ARIMA and SARIMA

The most popular model is the **ARIMA** (Auto-regressive Integrated Moving Average Model). ARIMA assumes that we have stationary trends (no seasonality):

$$
\text{ARIMA}(p, d, q) \ : \ X_t'^d = \underbrace{\sum\limits_{i=1}^p \phi_iX_{t-i}}_{\text{AR}} + \underbrace{\sum\limits_{i=1}^q \theta_i\epsilon_{t-i}}_{\text{MA}} + \epsilon_t
$$
...
We can choose $p$ and $q$ by first choosing a variety of $p$ and $q$, estimating each model with maximum likelihood estimation, and choosing the model (and $p$ and $q$) that has the least AIC or BIC value.

ARIMA is a non-seasonal models, since seasonality is technically non-stationary by definition. **SARIMA** models are the seasonal extension: we add seasons by doing seasonal differencing. This means taking $X_t$, and subtracting the $X$ value from the same point in the last season. Let us say each season is $S$ number of time periods long. So, a first-order seasonal difference would be $Z_t = X_t - X_{t-S}$.
...
Where $Z^{*d}$ is a $d$-order difference of $D$-order seasonal differences $Z_t'^D$. For example, if $d= 1$ and $D=1$:
