...
Vector Autoregressive (VAR) Models are a multivariate version of AR (auto-regressive) models. Here, we have multiple different time-series, and the VAR model allows relationships to occur between the variables in the model.

Let us take a simple VAR(1) model with only 1 previous time period, and 2 different time series $Y_t$ and $Z_t$:

$$
\begin{align}
Y_t & = \omega_y + \phi_{y1} Y_{t-1} + \phi_{y2}Z_{t-1} + \varepsilon_{yt} \\
Z_t & = \omega_z + \phi_{z1}Y_{t-1} + \phi_{z2}Z_{t-1} + \varepsilon_{zt}
\end{align}
$$

We can see that both $Y_t$ and $Z_t$ depend on their own lag in $t-1$, and the other's lag in $t-1$. We can also rewrite the above in matrix notation:

$$
\begin{pmatrix} Y_t\\ Z_t\end{pmatrix} = \begin{pmatrix} \omega_y \\ \omega_z \end{pmatrix} +
\begin{pmatrix} \phi_{y1} & \phi_{y2} \\ \phi_{z1} & \phi_{z2} \end{pmatrix}
\begin{pmatrix} Y_{t-1} \\ Z_{t-1}  \end{pmatrix} +
\begin{pmatrix} \varepsilon_{yt} \\ \varepsilon_{zt}\end{pmatrix}
$$

We can simplify this into matrix form to:

$$
VAR(1) \ : \ \mathbf y_t = \boldsymbol\omega + \boldsymbol\Phi \mathbf y_{t-1} + \boldsymbol\varepsilon_t
$$

And if we have VAR(p) model, we can generalise this to:

$$
VAR(p) \ : \ \mathbf y_t = \boldsymbol\omega + \boldsymbol\Phi_1 \mathbf y_{t-1} + \boldsymbol\Phi_2 \mathbf y_{t-2} + \dots + \boldsymbol\Phi_p \mathbf y_{t-p} + \boldsymbol\varepsilon
$$\
Note that there also exists a Vector Autoregressive Moving Average (VARMA) model, however, unlike the ARIMA which is more popular than the AR model, in the vector cases, VAR is more popular than VARMA. This is mainly to do with estimation challenges regarding VARMA.
