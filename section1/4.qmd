...
[The Central Limit Theorem is arguably, the most important part of statistical inference]{.underline}. Why? If we recall from [module 3](https://politicalscience.github.io/section0/3.html), the normal distribution has some key properties, which are very useful for statistical inference purposes.
...
A way to estimate the magnitude of the relationship between $X$ and $Y$ is the [best linear predictor]{.underline}. The best linear predictor is a linear function $Y = \alpha + \beta X$ that best predicts $Y$ with values of $X$

-   Essentially, it is a best fit line to our correlation
...
The "error", also called the residual, is basically, how far away is our line from the points in the data. It is the actual $Y$ value from our dataset, minus the predicted $Y$ value from our linear equation for any given point $(x, y)$. Mathematically, it is $y_i - \hat{y}_i$. For example, take the figure below, which shows the errors for every point in a dataset for 3 different best fit lines:

![](figures/4-residuals.png){width="100%"}

Essentially, we want to find the y-intercept $\alpha$ and slope $\beta$ of a linear line that [minimises the sum of squared errors]{.underline}.

The sum of squared errors (also called the residual sum of squares) is exactly what is sounds like. You take the errors (every red line in the figure above), and square them. Then you sum all of them up. Mathematically:

$$
RSS = \sum(y_i - \hat{y}_i)^2
$$

Why do we want to minimise the sum of squared errors? Well, the reason we square them is to get rid of the negative signs. This is because, the negative errors will simply cancel out the positive errors. But, we don't want that - we want some measure of the total amount of error. Thus, the square makes all the negatives positive, which allows us to measure the magnitude of the errors, not the direction.

<br />

Well, now that we know we want to minimise the sum of squared errors, how do we actually get the slope $\beta$ and y-intercept $\alpha$ for our linear model?

-   We of course, could try thousands/millions of different lines, and see which one has the lowest sum of squares

-   Or, we can use a mathematical estimator to estimate the optimal $\beta$ and $\alpha$

The most common estimator of the slope $\beta$ and y-intercept $\alpha$ is called the [Ordinary Least Squares (OLS) Estimator]{.underline}. We will start with this estimator, and the linear model as a whole, in the next module.
