...
-   [Saving Tree Predictions for Each Observation]
...
To create a bagging prediction model (**Y \~ X1 + X2 +...**), we can use the **randomForest()** command. In the example below:
...
However, we can get more detailed information on this model. See the importance plots section and prediction section for more information:
...
## Random Forest {data-link="Random Forest"}
...
The reason for this? Random Forest doesn't just bootstrap sample observations, it also samples independent variables, to further reduce the issue of variance.
...
We might be wondering, since we included many independent variables, which ones are most important in predicting the outcome?

To see the importance of each predictor variable, we can use the **varImpPlot()** function. Take note of the following:

-   **bagging** is the variable I saved the previous model to. You should replace this with the variable you stored your own bagging model in. You can use bagging or random forest models in this function.
...
-   Within the **predict()** function is **bagging**, the name of my bagging model. You can change this to whatever variable you saved your bagging/random forest model to.
