...
Lets run simple binomial logistic regression with a Binary Y (**Y \~ X1 + X2 + X3...**) . For non-binary Y, see multinomial and ordinal logistic later in this lesson. In the example below:

-   **logistic_reg** is the variable I am saving my regression model to. You can name this anything you want to.

-   **voc** is the Y variable (Dependent variable) I am using for the regression. This is what variable I am trying to predict. Replace this with the Y variable you intend on using. I included **as.factor()** so R treats this as a categorical variable (only necessary if your categorical variable is coded into numbers).

-   **personaltax** is the X1 variable (Independent Variable) I am using for the regression. Replace this with the X variable you intend on using.

-   **gini** is the X2 variable (Independent Varibale) I am using for the regression. Replace this with the X variable you intend on using.
...
-   To add more than 2 X independent variables, simply add in the formula **+ X3 + X4 ...**
...
We can interpret the significance levels (stars) and **Pr(\>\|z\|)** as we normally do with linear regressions.

-   If **Pr(\>\|z\|)** is less than 0.05, we can reject the null hypothesis, and conclude there is a significant relationship.
...
However, unlike linear regressions, **we cannot directly interpret the estimated coefficients**. These actual numbers do not mean anything meaningful to us. To interpret, we need to calculate the **odds ratios** of the regression (see below).
...
To interpret logistic regression coefficients, we need to calculate the odds ratios. We can do this with a few commands. In the example below:
...
If we want to calculate the 95% confidence intervals of the odds ratio estimates, we can create a table using **cbind()** that includes both coefficient values and confidence interval values. In the example below:
...
Multinomial logistic regression is used to predict a Y, which has more than 2 outcomes. We will need to install the **VGAM** package
...
We will run this regression on the Y variable Species, which has 3 distinct categories (**Y \~ X1 + X2 + X3...**) . The **vglm()** function will automatically set the last of these categories as the "reference category", so the probabilities are in comparisons to the reference category. In the example below:
