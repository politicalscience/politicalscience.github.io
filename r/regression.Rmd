...
-   An error is the difference between the actual value of $y$, and the predicted $\hat{\mathbf{y}}$ of the model. The squared error is the square of the difference. The sum of the squared errors is all of the squared errors for every value of $x$.
-   OLS methodology uses mathematics (beyond the scope of this guide) to find the optimal $\hat{\mathbf{\beta}}_0$ and $\hat{\mathbf{\beta}}_1$ that minimises the sum of the squared errors.
...
-   If your X variable is categorical/non-continuous/binary, please consult the section: [Factor X Variables (Non-Continuous X Variables)](#factor-x-variables-non-continuous-x-variables)
...
-   The $r^2$ value is the amount of variation in Y explained by the model.
...
Instead, we have to treat these variables as factor variables. For example, let us take the binary categorical X variable **democracy**.
...
-   Remember to use **as.factor()** on categorical variables when applicable (see section: [Factor X Variables (Non-Continuous X Variables)])
...
-   $\hat{\mathbf{\beta}}_p$ is the effect on $x_p$ on $y$, given all other $x$ are held constant.

Multivariate regression also uses the OLS framework to estimate paramters. However, unlike simple linear regression which can be illustrated with a line in a 2-dimensional graph, things can get very complicated mathematically/graphically, as every $x_p$ is a different dimension.
