...
-   [Factor X Variables (Non-Continuous X Variables)]
...
A simple linear regression is a method to create a prediction of Y, given a value of X (**Y \~ X)**. We do this by training the model on a dataset with both X and Y, and the computer will use Ordinary Least Squares (OLS) methodology to find the relevant parameters to create a linear line of best fit. OLS is only used on continuous Y variables (not categorical Y variables).

We conduct a simple linear regression (**Y \~ X**) in R as follows. Take note of the following:
...
We can also put this in graphical form with **ggplot** as follows. Take note of the following:
...
What are Factor/Non-Continuous X Variables? These are variables that are generally categorical - as in that there are a limited number of categories, and these categories are un-rankable (one is not inherently better/higher than another). These behave differently from numerical continuous variables (ex. 2 \> 1, 3 \> 2, etc.).
...
For these variables, we need to change how we do the regression. Why? Well, for a continuous numerical variable, the slope calculated from our OLS regression (B1) makes sense - for every increase of 1 in X, we expect a certain change in Y. However, we can't really say the same with categorical data.

-   Ex. if we included **company** as a numerical variable, that would imply that going from apple to microsoft is taking 1 step up, with the slope representing the change in Y. But the slope makes no sense, especially if we apply it to going from microsoft to nvidia, also 1 step up.

Instead, we need to include these as factor variables - essentially, each category is its own binary variable, with slope indicating the difference between in-category and out-category.
...
We can tell R to do this with the **as.factor** function, and putting that variable we want to treat as a factor inside of it. See the following:
...
Almost always in political science, we will need more than one X predictor variable. This is because very few phenomenon are caused by one variable only (**Y \~ X1 + X2 + X3...**). Thus, we need to control for other potential causes of Y in order to estimate the effect of our main X on Y.

In R, we can do this quite simply by adding onto the Single Linear Regression format. Take note of the following:
...
-   **socialexp** is the X2 variable (Independent Variable) I am using for the regression. This is one of two variables I am using to make the prediction. Replace this with the X2 variable you intend on using.
...
The Multiple Linear Regression we looked at before assumes that each variable has an independent effect (hence, why we put a + sign). That means, one variable's impact on Y does not affect another variable's impact on Y.

However, this isn't always the case. There are plenty of times where one variable's value will impact the strength the the relationship between another variable and Y. For example, there is a relationship between high speeds and accident frequency in cars. However, this relationship because far more dangerous if the roads are slippery. So, the slippery roads is **moderating/interacting with** the effect between high speeds and accident frequency.

We can implement moderating/interaction effects in R with the \* sign, where \* goes between the two X variables that affect each other (**Y \~ X1 \* X2 + X3 + X4...**). Take note of the following:
