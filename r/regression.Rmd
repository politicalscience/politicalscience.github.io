...
[Intuition and Theory](#1001) \| [Example in R](#1002) \| [Graphing Simple Linear Regression](#graphing)
...
Note how this is the same form as a standard linear equation: $y=mx+b$ . Our linear regression model will be a straight linear line, as the name implies.

<br />

OLS generates an prediction equation in the above form, through selecting $\hat{\mathbf{\beta}}_0$ and $\hat{\mathbf{\beta}}_1$. It selects these coefficient values through finding the coefficient values that minimises the **sum of squared errors** of the prediction model.
...
-   The sum of the squared errors is all of the squared errors for every value $i$ of $x$: $\sum (y_i-\hat{\mathbf{y_i}})^2$

<br />

OLS methodology uses the following formulas to find the optimal $\hat{\mathbf{\beta}}_0$ and $\hat{\mathbf{\beta}}_1$ that minimises the sum of the squared errors we just described:

-   $\hat{\mathbf{\beta}}_1 = \frac{\sum (x_i-\bar x)(y_i - \bar y)}{\sum(x_i-\bar x)^2}$

-   $\hat{\mathbf{\beta}}_0 = \bar y - \hat{\mathbf{\beta}}_1 \bar x$

I will not show the mathematical proof of these formulas, as that is beyond what is needed for this lesson. However, a quick google search will show you statistical proofs of the calculations.

For this guide, we will let R do the calculations of coefficients for us, instead of manually plugging in the numbers to these formulas.

<br />

Uses of linear regression in Political Science research:

-   Linear regression can help establish correlation/covariation between variables, which is essential to establishing causal relationships.

-   Linear regression is often used in hypothesis testing to determine if there is a significant relationship between 2 variables, that is not occurring because of chance.
...
### Graphing Simple Linear Regression {#graphing}
...
-   **X_variable** is the variable to plot on the X axis, and **Y_variable** is the variable to plot on the Y axis. *Replace these with the variables you want to use.*
...
-   For example, if we have the categorical variable **company**, with the vector $<microsoft,  apple,  google>$, a constant slope makes no sense. Why would an increase of 1 from microsoft to apple, have a constant depedent variable Y change the same as from apple to google? These companies aren't evenly spaced and ordered on a number line.
...
What if we have more than 2 categories in our X variable? What we need to do is essentially divide our X variable into X variables with 2 categories (so we can compare the difference!). For example, take the earlier example of the variable **company**, with values $<microsoft,  apple,  google>$, with values $<0, 1, 2>$
...
-   Let us take **microsoft** as our **reference category**. This will be the point of comparison for both our variables.
-   Now lets create a variable: **apple or not** ($x_1$). We will define $x_1 = 0$ as **not apple**, and $x_1 = 1$ as **apple**.
-   Now lets create a second variable: **google or not** ($x_2$). We will define $x_2 = 0$ as **not google**, and $x_2 = 1$ as **google**.
-   We don't need a variable for our reference category **microsoft**. This is because microsoft is simply when both $x_1, x_2$ are equal to $0$
...
Thus, the following is true about our coefficients:

-   The coefficient of $x_1$, $\hat{\mathbf{\beta}}_1$, is the difference between apple and microsoft (our reference).

-   The coefficient of $x_2$, $\hat{\mathbf{\beta}}_2$, is the difference between google and microsoft (our reference).

-   The prediction of our reference category, microsoft, is $\hat{\mathbf{\beta}}_0$

Note that we cannot compare microsoft and apple directly - to do this, we have to change the reference category to either on of them.

The same process works with any X with more than 2 categories. The number of variables to divide X into is $categories - 1$

<br />

Uses of Categorical X Variable regression in Political Science:

-   Many common variables used in Political Science take the form of categorical variables. For example: democracy or not; type of electoral system; country; etc.
...
We can insert this **as.factor()** function into anywhere where there is a variable in R. For example, let us do a regression, where we predict **personaltax** using the factored variable **voc**. R will automatically treat either $x = 0$ or the first category of the variable vector as the reference category.
...
# [Multivariate Linear Regression]{.underline} {#multivariate}
...
Uses of multivariate linear regression in Political Science research:

-   Since we can hold other variables constant, this allows us to focus on the effect of one independent variable, and treat the others as control variables. Controlling for alternative explanations is a key part in determining causality.

-   Political Science phenomena are rarely caused by just one variable. Thus, to make accurate predictions, multivariate linear regression is very useful, as it allows us to include many different potential causes and factors.
