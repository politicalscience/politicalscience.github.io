...
subtitle: "Chapter 6: Multiple Linear Regression - Estimation and Interpretation"
format:
    html:
        page-layout: article
        grid:
        ...
        toc-location: left
        toc-expand: true
        toc-title: "Chapter 6: Multiple Linear Regression - Estimation and Interpretation"
...
# 6.1: Motivation Behind Multiple Linear Regression

In [section 5.2](https://politicalscience.github.io/metrics/5.html#unbiasedness-of-ols-under-the-gauss-markov-theorem), we discussed how the simple linear regression with the OLS estimator is only unbiased under 4 conditions.
...
Why is this condition frequently violated? Recall from [section 4.1](https://politicalscience.github.io/metrics/4.html#the-simple-linear-regression-model), where we discussed that the error term $u_i$ represents the effect of anything else on $y$ other than our explanatory variable $x$.
...
-   Note: This concept will be further discussed in chapter 7 with omitted variable bias and Endogeniety.

<br />

How do we address this violation of the Zero Conditional Mean resulting from the extra variable $w$?

-   Well, there are actually several ways to do this, which we briefly mentioned at the end of [section 5.3](https://politicalscience.github.io/metrics/5.html#proof-of-unbiasedness-of-ols-under-gauss-markov).

The simplest way to address it is to include the extra variable $w$ into our regression as another explanatory variable.

-   This way, $w$ is another explanatory variable along with $x$, and is no longer in the error term $u$, solving our issue!
...
-   Multiple linear regression allows us to "control for the effect" of confounders. We will discuss this in section 6.5.
...
Same as in Simple Linear Regression, once we have estimated $\overrightarrow\beta$, we will have a best-fit **plane**, also called a fitted-values model (see [section 4.2](https://politicalscience.github.io/metrics/4.html#fitted-values-and-best-fit-lines)). The fitted values model takes the form:
...
# 6.3: Multiple Linear Regression with Linear Algebra
...
# 6.4: Mathematics of the Ordinary Least Squares Estimator
...
# 6.5: Regression Anatomy and Controlling for Confounders
...
Let us say we are interested in $x_1$ (this can be generalised to any explanatory variable $x_j$). Let us make $x_1$ the outcome variable of a regression with $x_2, ..., x_k$:
...
-   In other words, $\widetilde{r_{1i}}$ is the part of $x_1$ that cannot be explained by any other explanatory variable. (uncorrelated with them)
...
We can also standardise our interpretations as shown in [section 4.5](https://politicalscience.github.io/metrics/4.html#interpretation-and-standardisation).
...
# 6.6: Regression Anatomy Formula for OLS Estimation
...
We have already discussed the mathematical solution of OLS in relation to linear algebra (see section 6.4). However, we can also express the estimation solution of $\hat\beta_j$ in relation to the regression anatomy formula.

<br />

Let us start off with the OLS estimator for simple linear regression, which calculates the $\hat\beta_1$, the relationship between $x$ and $y$ (which we derived in [section 4.4](https://politicalscience.github.io/metrics/4.html#mathematics-of-the-ordinary-least-squares-estimator)):
...
$$
x_{ji} = \gamma_0 + \gamma_1 x_{2i} + ... + \gamma_{k-1}x_{ki} + \widetilde{r_{ji}}
$$
...
This formula will be very useful in analysing the properties of the OLS estimator (in chapter 7).
...
This explains intuitively why the Zero-Conditional mean assumption (discussed in [section 5.2](https://politicalscience.github.io/metrics/5.html#unbiasedness-of-ols-under-the-gauss-markov-theorem)), when violated, results in biased OLS estimates.
...
-   We will discuss this more when we discuss the Gauss-Markov conditions of Multiple Linear Regression in Chapter 7.

-   We will also discuss in Chapter 7, how much (quantitatively) our estimates will be biased when we fail to include a confounder $w$.
...
# 6.7: R-Squared and Goodness of Fit

In [section 4.6](https://politicalscience.github.io/metrics/4.html#r-squared-and-goodness-of-fit), we discussed R-squared as a measure of the goodness of fit of a model.
