...
In [section 2.7](https://politicalscience.github.io/metrics/causal/2.html#selection-bias-and-confounding-variables), we discussed how confounder $C$ affects which observations get the treatment, which introduces selection bias.
...
We can also use simple linear regression (see [chapter 1](https://politicalscience.github.io/metrics/causal/1.html)) and the OLS estimator to estimate causal effects in randomised experiments.
...
We can estimate the $\beta_0$ and $\beta_1$ with the OLS estimator to produce a fitted value equation (see [section 1.3](https://politicalscience.github.io/metrics/causal/1.html#fitted-values-and-the-sum-of-squared-errors) and [section 1.4](https://politicalscience.github.io/metrics/causal/1.html#mathematics-of-the-ordinary-least-squares-estimator))

$$
\hat y_i = \hat \beta_0 + \hat\beta_1D_i
$$

What do $\hat\beta_0$ and $\hat\beta_1$ mean? We can "solve" for these interpretations. Assume $D$ has two categories $D=0$ and $D=1$:
...
Thus, we need some mechanism to account for sampling variability and how rerunning the experiment might result in different results. We do this with sampling distributions and standard errors, which we covered in [section 1.6](https://politicalscience.github.io/metrics/causal/1.html#uncertainty-and-standard-errors).

<br />

### Standard Errors

While we covered standard errors in [section 1.6](https://politicalscience.github.io/metrics/causal/1.html#uncertainty-and-standard-errors), things differ slightly with the difference-in-means estimator.
...
We discussed the intuition of the hypothesis tests in [section 1.7](https://politicalscience.github.io/metrics/causal/1.html#hypothesis-testing-and-statistical-inference).
...
After we have a test statistic, we need to consult a t-distribution, and calculate the p-value. See [section 1.7](https://politicalscience.github.io/metrics/causal/1.html#hypothesis-testing-and-statistical-inference) for more details on this.
...
We have a causal estimate $\hat{\tau}_{ATE}$ already. However, the issue, as we described in [section 3.4](https://politicalscience.github.io/metrics/causal/3.html#uncertainty-in-estimates-and-standard-errors), is that if we run another random assignment, we will get a slightly different causal estimate.
...
The "key" thing about confidence intervals is if the interval contains the number 0. This is because if 0 is in the interval, that means 0 is a plausible value of the ATE.
