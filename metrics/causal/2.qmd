---
title: "Chapter 2: Randomised Controlled Trials as the Gold Standard"
...
format:
    html:
        ...
        toc-location: left
        toc-expand: true
        toc-title: "Chapter 2: Randomised Controlled Trials as the Gold Standard"
...
In [section 1.6](https://politicalscience.github.io/metrics/causal/1.html#selection-bias-and-confounding-variables), we discussed how confounder $C$ affects which observations get the treatment, which introduces selection bias.
...
Thus, we have some uncertainty with our causal estimate - re-running the experiment might result in a different answer. The ATE we have calculated is only our specific sample average treatment effect (SATE), notated $\widehat{ATE}$.
...
Imagine that we take a sample from a population (or some random assignment mechanism). Then, we find the average treatment effect of the sample $\widehat{ATE}$.
...
According to c**entral limit theorem**, the sampling distribution approximates that of a normal distribu- tion (or t-distribution if our sample size is small). We know that a normal distribution is defined by two parameters - mean and variance.
...
Alternatively, we can use regression to estimate the difference-in-means estimator (which will be covered in the next chapter):
