...
-   If $x$ is categorical, see section 1.7 for interpretation.
-   If $y$ is binary, see section 1.8 for interpretation.
...
# 1.7: Binary and Categorical Explanatory Variables
...
Additional note: The coefficient $p$-values of $\beta_j$ (discussed in section 2.7) are a difference-of-means test between two categories, and not a statistical significance test of the entire categorical variable.
...
# 1.8: Binary Outcome Variables and the Linear Probability Model
...
-   We can also interpret this in terms of percent-chance, by multiplying $\hat\beta_1$ by 100.
:::
...
-   We can also interpret this in terms of percent-chance, by multiplying $\hat\beta_0$ by 100.
:::
...
# 1.9: Polynomial and Logarithmic Transformations
...
-   [If the coefficient of $\beta_2$ is statistically significant, we can conclude that there is a non-linear relationship between $X$ and $Y$]{.underline}. (we will discuss this in chapter 2).
...
Each higher order coefficient, if statistically significant (we will discuss this in the next chapter), indicates that the relationship between $X$ and $Y$, is not of the previous highest power.
...
y_{i, \ x_i = e^A + x} - y_{i, \ x_i = x} & = \beta A\end{split}
$$
