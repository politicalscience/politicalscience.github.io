...
Once we have our OLS estimates $\hat\beta_1$ and $\hat\beta_1$ (explained in [section 4.4](https://politicalscience.github.io/metrics/4.html#mathematics-of-the-ordinary-least-squares-estimator)), we can obtain both our fitted values, and the OLS residuals $\hat u_i$:
...
-   The error is prior to the estimation, if we recall from the simple linear model, it is the distance of $y_i$ to the conditional expected value $E(y_i | x_i)$.
...
Note: This does not mean that the actual regression line must be linear. We will explore how we can mantain linear parameters while having non-linear relationships in chapter 9.
...
That means each $i$ is a random draw from the population, which allows us to go from $y = \beta_0 + \beta_1 x + u$ to $y_i = \beta_0 + \beta_1 x_i + u_i$.
...
$$
\begin{split}
& = \sum\limits_{i=1}^n (x_i - \bar{x})y_i \\ & = \sum\limits_{i=1}^n(x_i - \bar{x})(\beta_0 + \beta_1 x_i + u_i) \\& = \beta_0 \sum\limits_{i=1}^n(x_i - \bar{x}) + \beta_1 \sum\limits_{i=1}^n(x_i - \bar{x}) x_i + \sum\limits_{i=1}^n (x_i - \bar{x}) u_i \\& = 0 + \beta_1 \sum\limits_{i=1}^n(x_i - \bar{x})^2 + \sum\limits_{i=1}^n(x_i - \bar{x})u_i\end{split}
$$
...
Now we need to find the expectation $E(\beta_1)$. Thus, we have this equation:
...
Now knowing this, we can complete the proof:
...
But you might notice, we have solved for $E(\hat\beta_1 |x)$, and not $E (\hat\beta_1)$.
...
1.  Omitted Variable Bias. This is because if we have any other variable correlated with $y$ and $x$ in the error term $u_i$, $E(u|x)$ will be violated. (we will discuss this more in chapters 6 and 7)
...
1.  Add control variables with multiple linear regression (discussed in chapters 6 and 7)
...
When $Var(u)$ is not constant (and changes with the value of $x$, we have something called **heteroscedasticity**.
...
What is variance? From [section 1.5](https://politicalscience.github.io/metrics/1.html#variance-and-standard-deviation), we know that variance is:
...
There is one issue: we know that $\sigma^2 = E(u^2)$ (see section 5.5). However, we do not actually know the value of $E(u^2)$! Remember, that is the error term $u$ - while we only know the residual term $\hat u$.
...
$$
\hat{se}(\hat\beta_1) = \frac{\hat\sigma}{\sqrt{SST_x}}
$$
...
$$
\hat\beta_1 - 1.96 \hat{se}(\hat\beta_1), \ \hat\beta_1 + 1.96 \hat{se}(\hat\beta_1)
$$
