...
Important note, the residual $\hat u_i$ is different from the error $u_i$. Error $u_i$ is based on the true parameters $\beta_0$ and $\beta_1$, not our OLS estimates $\hat\beta_0$ and $\hat\beta_1$.
...
This also implies that the error term $u$ an $x$ are uncorrelated with each other (since no matter $x$, $u$ takes the same value 0). Thus:
