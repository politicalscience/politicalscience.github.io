...
subtitle: "Chapter 4: Simple Linear Regression - Estimation and Interpretation"
format:
    html:
        page-layout: article
        grid:
        ...
        toc-location: left
        toc-expand: true
        toc-title: "Chapter 4: Simple Linear Regression - Estimation and Interpretation"
...
For example, take the figure below - not all points are on the best-fit line. $u_i$ represents the distance of points from the best fit line:
...
# 4.2: Fitted Values and Best-Fit Lines

We have discussed the form a simple linear regression takes: $y = \beta_0 + \beta_1 x + u_i$.
...
1.  The estimate $\hat\beta_1$ is the estimate of the slope of the linear equation. That means $\hat\beta_1$ explains the amount of change in $y$, given a one unit increase in $x$. If we are interested in describing the relationship between two variables, $\hat\beta_1$ is what we will focus on.
2.  $\hat y$ is the predicted values of $y$, which means we can also use the fitted values to make predictions. Just plug in values of $x$, and the fitted values equation will output a prediction $\hat y$. While this course does not focus on prediction, the subsequent course on Applied Machine Learning will dive much more into this topic.
...
Thus, one way we can fit an accurate line is to find the best-fit line that minimises the sum of squared errors (SSE).
...
# 4.4: Mathematics of the Ordinary Least Squares Estimator

The Ordinary Least Squares (OLS) Estimator estimates the coefficients $\beta_0$ and $\beta_1$ by finding the values of $\beta_0$ and $\beta_1$ that result in the line with the smallest sum of squared errors (as discussed in the last section).
...
Note: in the next chapter, we will discuss if the OLS estimator is a good estimator or not. For now, we just care about the mechanics of the estimator.
...
# 4.5: Interpretation and Standardisation
...
# 4.6: R-Squared and Goodness of Fit
...
# 4.7: OLS Regression as a Conditional Expectation Function
