...
The Maximum Likelihood Estimator is great, however, it can be computationally intensive. A series of estimators called **least squares** is another way to estimate population estimands $\theta$. However, this classs of estimators can only be applied in one specific model: the classical linear model.
...
Where $\beta_0, \dots, \beta_p$ are a set of population parameters that determine how $\mu_Y$ changes in respect to explanatory variables $\set X$. Our goal with least squares estimation will be to estimate these parameters.
...
There are a set of **five** **assumptions** that the above model has to meet to be considered a **classical linear model**.
...
We will first explore the **Ordinary Least Squares Estimator**, the most famous estimator in statistics that is applicable when all 5 of the classical assumptions are met.

Then, we will introduce estimators for when some of the classical conditions are not met. First, we will introduce the **Generalised Least Squares** estimator for when the spherical errors assumption is violated. Then, we will introduce the **Instrumental Variables** estimator for when exogeneity is violated.

<br />

## Ordinary Least Squares

How can we derive sample estimates $\hat{\b\beta}$ of parameters $\b\beta$? One logical way is to see what values of $\b{\hat\beta}$ will produce the closest predictions of $\b y$ values to the actual sample values. Our predicted $\b y$ values are $\hat{\b y} = \b X \hat{\b\beta}$.

OLS is an estimation process that finds the values $\hat{\b\beta}$ by the $\hat{\b\beta}$ values that minimise the **sum of squared residuals** (SSR), the difference between $\b y$ and predicted $\hat{\b y}$ squared.
...
& =  \b y^\top \b y - 2 \hat{\b\beta}^\top \b{X^\top y} + \underbrace{\hat{\b\beta}^\top \b{X^\top X} \hat{\b\beta}}_{\text{quadratic term}}
\end{align}
$$
...
Now, solving for $\hat{\b\beta}$, we can get our OLS estimates:
...
To understand OLS, let us first define 2 important matrices:
...
We can prove $\color{red}{\b P}$ is an idempotent matrix.
...
We can show $\b M$ and $\b P$ are orthogonal, i.e. $\b{P^\top M} = 0$. First, recall that $\b P$ is a symmetrical matrix, meaning, $\b P^\top = \b P$. Thus,
...
## OLS Projection and Fit

Recall our predictions $\hat{\b y} = \b X \hat{\b\beta}$. Using our OLS solution, and the plugging in the definition of projection matrix $\color{red}{\b P}$ (@def-projectmatrix), we can find
...
To understand the parameter estimates $\hat\beta$ of OLS, we want to study the partitioned regression model. A partitioned regression model is when we split up our matrices/vectors in our classical linear model $\b y = \b{X\beta} + \b\eps$.
...
We know that an estimator has two finite sample properties: unbiasedness (@def-unbiased) and variance (@def-varest)., and has the asymptotic property of consistency. Before we explore these properties, let us first transform our OLS estimates (@def-ols) into something more easily manipulable.
...
The Ordinary Least Squares estimator is asymptotically consistent (@def-consistency), under the assumptions of [linearity](#assumption-1-linearity-in-parameters), [i.i.d.](#assumption-2-independent-and-identically-distributed-i.i.d.), [no perfect multicollinearity](#assumption-3-no-perfect-multicollinearity), [weak exogeneity](#def-weakexog), and [spherical errors](#assumption-5-spherical-errors). Mathematically:
...
For the classical linear model, one of the assumptions was spherical errors (@def-sphericalerrors). This was an assumption made on the variance-covariance matrix of error term $\eps_t$. The assumption was split into two sections:

1.  **No Autocorrelation**: The covariance of any two error terms $Cov(\eps_i, \eps_j) = 0$.
2.  **Homoscedasticity**: The variance of every error term $\eps_t$ for every observation $t$ is constant with the same variance $\sigma^2$. The variance of $\eps_t$ does not depend on the values of regressors $X_1, \dots, X_p$.
...
We mentioned that if spherical errors is violated, OLS is no longer the linear unbiased estimator with the least variance. Instead, another estimator, the Generalised Least Squares estimator, is the best linear unbiased estimator.
...
The Cochrane-Orcutt Estimator is a way to estimate GLS assuming autocorrelation of a specific form (Autoregressive 1).
...
The weighted least squares (WLS) estimator is a FGLS when we have only conditional heteroscedasticity, and we believe that conditional heteroscedasticity to be in a specific form (a function of some explanatory variables).
...
![](images/clipboard-4129117926.png){fig-align="center" width="32%"}
...
Below is the derivation of the instrumenetal variables estimator, and alternative IV estimator (the 2SLS), and some important properties.
