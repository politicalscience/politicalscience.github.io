...
::: {.callout-note collapse="true" appearance="simple"}
## Hierarchical/Clustered Data and Panel Data
...
If we have spatial data, a region's neighbours may have some effect on a region's own outcomes:

1.  The outcome $y$ of a region might be affected by a neighbouring region's $y$ values (Lag $y$)
2.  The outcome $y$ of a region might be affected by a neighbour's $X$ values (lag $X$).
3.  The residuals $u$ of a region might be affected by the residuals in a neigbouring region (spatial autocorrelation).

::: {.callout-note collapse="true" appearance="simple"}
## Weights Matrix
:::

We define neighbours with a weights matrix $\mathbf W$. The largest spatial model is the **Manski Model**:
...
-   $\rho$ (scalar) is the lag $y$ coefficient. For every one unit increase in the average value of $y$ in neighbouring regions, there is an expected $\rho$ unit change in $y$ in the region, holding all else constant.
-   $\boldsymbol\beta$ (vector) is our normal OLS coefficients $\beta_0, \dots, \beta_k$, and would be interpreted in the same way.
-   $\boldsymbol\theta$ (vector) is the lag $X$ coefficients $\theta_0, \dots, \theta_k$. For every one unit increase in the average $x_j$ of neighbouring regions, there is an expected $\theta_j$ unit change in $y$ in the region, holding all else constant.
-   $\lambda$ (scalar) is spatial autocorrelation coefficient. Unobserved factors shared by neighbouring regions contribute to a $\lambda$-unit change in $y$, holding all else constant.
...
The models are estimated with Maximum Likelihood Estimation (discussed in next chapter), and model selection can be done with a combination of eliminating insignificant portions, and AIC and BIC.
