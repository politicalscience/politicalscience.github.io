...
One thing we might be interested in is how well our model $\mathbf{Py}$ explains the actual $\mathbf y$. One way we can do this is the scalar product: the scalar product $\mathbf y^\mathsf{T}\mathbf{Py}$ describes the shadow the actual $y$ casts on our projected model. However, this value will change based on the scale of our $y$ variable. Thus, we will divide it by $\mathbf y^\mathsf{T}\mathbf y$, which is the "maximum" shadow possible (perfect shadow). This ratio is called $R^2$.
...
$$
\begin{align}
E(y_i|x_i) & = \underbrace{0 \times P(y_i = 0|x_i) \ + \ \times P(y_i = 1|x_i)}_{\text{a weighted avg. formula}} \\
& = P(y_i=1|x_i)
\end{align}
$$
...
$$
P(y_i = 1|x_i) = \beta_0 + \beta_1x_{1i} + \dots + \beta_kx_{ki} + u_i
$$
...
-   Where $y_{it}$ is the $y$ value for the $i$th observation within the $t$th cluster.
-   Where $D_{i2}, D_{i3}, \dots, D_{im}$ are dummy variables for clusters $2, \dots, m$. Cluster 1 is the reference category.

We essentially add a unique intercept term for every cluster, accounting for the average differences in $y$ between each category. $\beta_{00}$ is the intercept for the reference category 1. $\beta_{00} + \beta_{0i}$ is the intercept for the $i$th category.
...
-   Where $y_{it}$ is the $y$ value of the $i$th unit at time period $t$.
...
1.  **Lag y**: The $y$ of a region might be affected by a neighbouring region's $y$ values. For example, one state having a high crime rate may affect another nearby state's crime rate as criminals can move across the border.
2.  **Lag X**: The $y$ of a region might be affected by a neighbour's $X$ values. For example, one state's low vaccination rate could affect the prevalence of a disease in a nearby state.
...
$$
\mathbf y = \underbrace{\rho\mathbf W\mathbf y}_{lag \ y} + \mathbf X\boldsymbol\beta + \underbrace{\mathbf{WX}\boldsymbol\theta}_{lag \ x} + \underbrace{\lambda \mathbf{Wu}}_{autocorr.} + \boldsymbol\varepsilon
$$
...
1.  Keleijian-Prucha Model: $\mathbf y = \rho \mathbf W \mathbf y + \mathbf X \boldsymbol\beta + \lambda \mathbf {Wu} + \boldsymbol\varepsilon$.
2.  Spatial Durbin Model: $\mathbf y = \rho \mathbf{WY} + \mathbf X \boldsymbol\beta + \mathbf{WX}\boldsymbol\theta + \boldsymbol\varepsilon$.
3.  Spatially Lagged X Model: $\mathbf y = \mathbf X \boldsymbol\beta + \mathbf {WX} \boldsymbol\theta + \boldsymbol\varepsilon$.
4.  Spatial Lag Model (Spatial Autoregressive): $\mathbf y = \rho \mathbf{Wy} + \mathbf X \boldsymbol\beta + \boldsymbol\varepsilon$.
5.  Spatial Error Model: $\mathbf y = \mathbf X\boldsymbol\beta + \lambda \mathbf{Wu} + \boldsymbol\varepsilon$.
