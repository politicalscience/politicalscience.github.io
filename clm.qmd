...
$$
\E(Y_i|X_i) = \beta_0 + \beta_1 X_{i1} + \dots + \beta_p X_{ip} \ = \ x_i^\top\beta
$$

Where $\beta_0, \dots, \beta_p$ are population parameters to be estimated, and vector $x_i^\top = (1, X_{i1}, \dots, X_{ip})$ and vector $\beta = (\beta_0, \dots \beta_p)$.

We can also specify linear regression not as the expected $Y$ as above, but for each observation of $Y_i$:

$$
\begin{align}
Y_i & = \beta_0 + \beta_1X_{i1} + \dots + \beta_pX_{ip} + \eps_i \\
& =   \beta_0 + \sum\limits_{j=1}^p \beta_jX_{ij} + \eps_i \ = \ x_i^\top\beta + \eps_i
\end{align}
$$
...
Or, we can impose a condition of normality of the error terms: $\eps | X \sim \mathcal N(0, \sigma^2 I)$. This will ensure the sampling distribution of $\hat\beta$ is normally distributed. (Note, the mean 0 and variance $\sigma^2 I$ come from the [classical assumptions](#the-classical-assumptions) of zero-conditional mean and spherical errors). However, this condition is very restrictive, so we typically rely on the reliable central limit theorem.
