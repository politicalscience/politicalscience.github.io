...
$$
\underbrace{E(y_i|x_i)}_{\text{expected } y_i} = \underbrace{\beta_0}_{\text{intercept}} + \underbrace{{\beta_1}}_{\text{Coefficient}}x_{1i} + \dots + \beta_kx_{ki}
$$

The linear model can also be specified for any specific outcome value $y_i$:

$$
\underbrace{y_i}_{y \text{ for } i} = \underbrace{\beta_0}_{\text{intercept}} + \underbrace{\beta_1}_{\text{coefficient}}x_{1i} + \dots + \beta_k x_{ki} + \underbrace{u_i}_{\text{error}}
$$
...
$$
se(\boldsymbol{\hat\beta}) = \hat\sigma \sqrt{(\mathbf X^T \mathbf X)^{-1}}
$$
