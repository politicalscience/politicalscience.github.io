...
$$
\begin{align}
L(\boldsymbol\theta, y_1, \dots y_n) & = f(y_1;\boldsymbol\theta) \times f(y_2;\boldsymbol\theta) \times \dots \times f(y_n;\boldsymbol\theta) \\
& = \prod\limits_{i=1}^n f(y_i, \boldsymbol\theta) \\
\end{align}
$$
...
$$
\begin{align}
\log L(\boldsymbol\theta, y_1, \dots y_n) & = \log \left(\prod_{i=1}^n f(y_i; \theta) \right) \\
& = \log[f(y_1, \boldsymbol\theta) \times f(y_2, \boldsymbol\theta) \times \dots \times f(y_n, \boldsymbol\theta)] && \text{(expand product notation)} \\
& = \log[f(y_1, \boldsymbol\theta)] + \log [f(y_2, \boldsymbol\theta)] + \dots + \log[f(y_n, \boldsymbol\theta)] && \text{(property of logs)} \\ 
& = \sum\limits_{i=1}^n \log[f(y_i, \boldsymbol\theta)] && \text{(condense into sum)}
\end{align}
$$ {#eq-loglike}
...
1.  The maximum likelihood estimator is [asymptotically consistent](quant1.qmd#asymptotically-consistent-estimators) $plim(\hat\theta_{MLE}) = \theta$.
2.  The maximum likelihood estimator is [asymptotically normal](quant1.qmd#asymptotically-consistent-estimators), which allows for statistical inference.
...
$$
f(y) = \frac{1}{\sqrt{2\pi\sigma^2}} e^{\left( -\frac{(y- \mu)^2}{2 \sigma^2}\right)}, \quad f(y_i|\beta_0, \beta_1, \sigma^2) = \frac{1}{\sqrt{2\pi\sigma^2}} e^{\left( -\frac{1}{2 \sigma^2}(y_i - \beta_0 - \beta_1 x_i)^2\right)}
$$
...
$$
\begin{align}
\log L(y_i | \beta_0, \beta_1 \sigma^2) & = \log \left(\prod\limits_{i=1}^n f(y_i | \beta_0, \beta_1, \sigma^2) \right) \\
& = \sum\limits_{i=1}^n \log f(y_i, \beta_0, \beta_1, \sigma^2) && (\text{from eq. (8)}) \\
