...
Experiments are a research design where the [assignment mechanism](quant2.qmd#assignment-mechanism) is controlled by the researcher.
...
Let us return to our [naive estimator](quant2.qmd#naive-estimator-and-selection-bias), and our problem of selection bias. Using the above property in @eq-independence, we can simplify:
...
We can also estimate the $\tau_{ATE}$ with a [regression](quant1.qmd):

$$
Y_i = \hat\gamma + \hat\tau D_i + \hat\varepsilon_i
$$

Remember that OLS is the best approximation of the [conditional expectation function](quant1.qmd#conditional-expectation-function) $\E(Y_i|X_i)$. Thus, we can write the regression as:
...
We do not need to include covariates. This is because randomisation allows us to meet the conditions of random sampling and zero-conditional mean neccesary for the [unbiasedness of OLS](quant1.qmd#unbiasedness-of-ols).
