...
OLS minimises the [sum of squared residuals](https://politicalscience.github.io/quant2.html#estimation-process) $S(\hat{\boldsymbol\beta})$ - the differences between the actual $\mathbf y$ and our predicted $\hat{\mathbf y}$:
...
Matrix $\color{red}{\mathbf P}$, called the **projection matrix**, is a matrix operator that performs the operation $\mathbf y \rightarrow \hat{\mathbf y}$.
...
Matrix $\color{purple}{\mathbf M}$, called the **residual maker**, is a matrix operator that performs the operation $\mathbf y \rightarrow \hat{\mathbf u}$.

Both $\color{red}{\mathbf P}$ and $\color{purple}{\mathbf M}$ are symmetric matrices - i.e. $\mathbf P^\mathsf{T} = \mathbf P$. They are also both indempotent matrices - i.e. $\mathbf {PP} = \mathbf P$. We can prove this second statement using the first (I will only do it for $\mathbf P$, but the same applies for $\mathbf M$:

$$
\begin{align}
\mathbf{PP} & = [\mathbf X (\mathbf X^\mathsf{T} \mathbf X)^{-1} \mathbf X^\mathsf{T}] [\mathbf X (\mathbf X^\mathsf{T} \mathbf X)^{-1} \mathbf X^\mathsf{T}] \\
& = \mathbf X (\mathbf X^\mathsf{T} \mathbf X)^{-1} \underbrace{\mathbf X^\mathsf{T} \mathbf X (\mathbf X^\mathsf{T} \mathbf X)^{-1}}_{= \ \mathbf I} \mathbf X^\mathsf{T} \\
...
That means, by the definition of [vector spaces](linear.qmdl#spanning-vectors-and-dimension), that our explanatory variable vectors $\mathbf x_j$ span a space that includes our fitted values vector $\hat{\mathbf y}$.
...
From here on, [homoscedasticity and heteroscedasticity](#error-variance-and-homoscedasticity) matter. Let us first start by deriving variance with homoscedasticity, using the definition given by @eq-homo :
...
Since $\sigma^2 \mathbf C (\mathbf I - \mathbf X(\mathbf X^\mathsf{T}\mathbf X)^{-1}\mathbf X^\mathsf{T})\mathbf C^\mathsf{T}$ is positive semi-definite (I will not prove this, but it is provable), we know that $Var(\tilde{\boldsymbol\beta}|\mathbf X) > Var(\hat{\boldsymbol\beta}|\mathbf X)$. Thus, OLS is BLUE under the Gauss-Markov Theorem.
